...
if(isset($_POST["upload"]))
{
  ...
  {   
      ...
      $check_query = mysqli_query($connect, "SELECT * FROM uploaded_projects WHERE UserId='$a1' AND project_Id='{$row1['project_Id']}'");
      ...
      if(isset($filename) && !empty($filename)){
          ...
          if($filename == ""){
              ...
              $folder  = "../Submissions/".$row1[0]."/".$a1."_receipt.".$ext;
              $folder1 = "../Submissions/".$row1[0]."/".$a1."_receipt.".$ext; 

          }

          if (move_uploaded_file($tempname, $folder))  { 
              $msg = "File uploaded successfully"; 
              $query2=mysqli_query($connect,"INSERT into uploaded_projects (UserId, project_Id) values('$a1',$row1[0])") or die("Error: " . mysqli_error($connect));
              $sql = "UPDATE `uploaded_projects` SET `file_path` = '$folder1' WHERE `UserId` = '$a1' AND `project_Id` = $row1[0]"; 
              $qzzz = mysqli_query($connect, $sql); 
              echo "<h3><center>".$msg."</center></h3>".'<center><h3>'."Redirecting to Projects Page.....".'</center></h3>';
              echo "<script>setTimeout(\"location.href = 'register.php';\",2500);</script>";
          }
          else{ 
              $msg = "Failed to upload Submissions/ receipt"."<br>"; 
              echo $msg;
              echo "<script>setTimeout(\"location.href = 'submission_page.php';\",2500);</script>";
          }
      }
  }
  $connect->close();    
...
echo "<h3 style = 'color: green;'> Upload the zip file of Project file (Max 2048 KB)</h3>";
    ...
    <input type="submit" name="upload" id="file_path" class="btn btn-warning" value=Upload />
    </form>
</div>
</body>
</html>
