...
if (isset($_POST["review"])) {
    ...
    if (mysqli_connect_errno()) {
        ...
        $project_Id = $row1[0];
        $a1 = $_SESSION["id"];
        $query2 = mysqli_query($connect, "INSERT into review (Review_Description, Review_Title, UserId, project_Id) values('$gh_2','$gh_1','$a1',$project_Id)") or die("Error: " . mysqli_error($connect));
    }
    $connect->close();
}
...
if (isset($_POST["deregister"])) {
    ...
    $a1 = $_SESSION["id"];
    if (!$row1[1]) {
        $query2 = mysqli_query($connect, "DELETE from applicants_list where UserId='$a1' and project_Id=$row1[0] ") or die("Error: " . mysqli_error($connect));
    } else {
        $query3 = mysqli_query($connect, "Select upload from applicants_list where UserId='$a1' and project_Id=$row1[0] ") or die("Error: " . mysqli_error($connect));
        $upl = mysqli_fetch_array($query3);
        if (!unlink($upl[0])) {
            echo $upl[0] . "cannot be deleted due to an error";
        } else {
            $query4 = mysqli_query($connect, "DELETE from applicants_list where UserId='$a1' and project_Id=$row1[0] ") or die("Error: " . mysqli_error($connect));
        }
    }
    $connect->close();
}
...
<body>
    ...
    if (mysqli_connect_errno()) {
        ...
        $query1 = mysqli_query($connect, "SELECT COUNT(UserId) FROM applicants_list WHERE project_Id={$row1['project_Id']}") or die("Error: " . mysqli_error($connect));
        $row = mysqli_fetch_array($query1);
        $c = $row[0];
        $a1 = $_SESSION["id"];
        $d1 = $_SESSION["project_name"];
        $query1 = mysqli_query($connect, "SELECT * FROM applicants_list WHERE UserId='$a1' and project_Id ={$row1['project_Id']}") or die("Errorb: " . mysqli_error($connect));
        $row1 = mysqli_fetch_array($query1);
        $flag = 1;
        ...
        //     $queryx = mysqli_query($connect, "UPDATE projects as p SET p.Status_Id = 1 WHERE project_Id =$project_Id") or die("Errorb: " . mysqli_error($connect));
        //     $rowst['Status_Id'] = 1;
        // }
        if ($c == $cc) {
            $query3 = mysqli_query($connect, "UPDATE projects as p SET p.Status_Id = 4 WHERE project_Id =$project_Id") or die("Errorb: " . mysqli_error($connect));
        }
        if (($rowst['Status_Id'] == 2) || ($rowst['Status_Id'] == 3) || ($rowst['Status_Id'] == 4) ) {
            $flag = 0;
        }
        if ((is_array($row1))) {
    ?>
        ...
        ?>
            <button type="button" class="btn btn-success">
                ...
                $x = mysqli_query($connect, "SELECT s.Status FROM projects as p JOIN project_status as s
                        ON p.Status_Id = s.Status_Id  WHERE project_Id =$project_Id") or die("Errorb: " . mysqli_error($connect));
    ...
    if (mysqli_connect_errno()) {
        ...
        $row1 = mysqli_fetch_array($query1);
        $project_Id = $row1['project_Id'];
        $query1 = mysqli_query($connect, "Select Review_Title,Review_Description,UserId,Review_Id,Timestamp from review where project_Id=$project_Id") or die("Error: " . mysqli_error($connect));
        $count = 1;
